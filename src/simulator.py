...
class Simulator(Server):
    ...
    def setup_entity_manager(self):
        ...
        cmd = f'cd {PROJECT_PATH}; source {VENV_PATH}; cd src; python entity_manager.py --host 0.0.0.0 --port {self._port} --secret {self._entity_managers[manager_address]} > ../logs/entity_manager.out 2>&1; echo Running EntityManager on process: $! >> ../logs/entity_manager.out'
    ...
    def setup_controller_manager(self):
        ...
        cmd = f'cd {PROJECT_PATH}; source {VENV_PATH}; cd src; python controller_manager.py --host 0.0.0.0 --port {self._port} --secret {self._controller_managers[manager_address]} > ../logs/controller_manager.out 2>&1; echo Running ControllerManager on process: $! >> ../logs/controller_manager.out'
