...
SRC_VENV = f"source /users/pgrad/{USER}/projects/scalable_project_3/.venv/bin/activate"
...
def setup_stations(num_stations: int, server_address: str, port: str, logdir: str):
    ...
    for _ in range(num_stations):
        ...
        cmd = f'{SRC_VENV}; python ~/projects/scalable_project_3/start_station.py {server_address} {port} {station_id} > {Path(logdir).absolute()}/{station_id}.out 2>&1; echo \nRunning on process: $?'
